...
function UpdateDesk({ token }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await apiClient.post(`/desks/${formData.desk_id}`, formData);
            toast.success(response.data.message || "Desk updated successfully!");
        } catch (error) {
            toast.error(error.response?.data?.message || "Failed to update desk.");
        }
    };
        ...
        <div>
            ...
            <div className="flex items-center justify-center">
                <form onSubmit={handleSubmit} className="w-full max-w-lg bg-white rounded-lg p-6">
                    ...
                    <div className="flex flex-col justify-center mb-4">
                        <label className="block text-gray-700 text-lg font-bold mb-2">New Desk Number:</label>
                        <input
                        type="number"
                        name="desk_number"
